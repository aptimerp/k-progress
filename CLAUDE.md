...
- **핵심 목적**: "지금 얼마를 벌고 있고, 앞으로 얼마를 벌 것인가?" — 재무·원가·매출·수익성 관제탑
- **담당 PL**: 김은지

## 기술 스택 (K-Progress)
- **언어**: Python
- **프레임워크**: Streamlit (SPA 구조 — 사이드바 내비 + 메인 영역 교체)
- **DB**: SQLite (로컬) — ERP 연동 데이터 시뮬레이션 테이블 포함
- **배포**: 미정
- **아키텍처**: Backend(데이터 모델)와 Frontend(UI) 느슨한 결합 — 확장성 우선

## ERP 연동 구조
- DB 엔진: MSSQL 2022 (Always On AG) — 기존 ERP
- K-Progress는 ERP 데이터를 **SQLite로 시뮬레이션** (실제 연동 전 단계)
- 실제 연동 시: KUMKANG_API_URL 환경변수 경유 (SAC API)
...
## 웹 프리뷰 자동 표시 규칙 (필수)
- `app.py` 또는 `pages/*.py` 파일을 수정한 후에는 **반드시** 웹 프리뷰를 보여줄 것
- 순서:
  1. `mcp__Claude_Preview__preview_start` 호출 (configName: "k-progress", 이미 실행 중이면 skip)
  2. `mcp__Claude_Preview__preview_screenshot` 호출해서 스크린샷 캡처
  3. 스크린샷을 사용자에게 보여주며 변경 결과 설명
